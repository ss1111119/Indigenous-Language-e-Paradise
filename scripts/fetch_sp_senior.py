"""
句型篇高中版：抓取全 42 方言 × 47 類別（8 種題型），直接寫入 corpus.db。

資料來源：
  - extension/sp_data/senior/classView.xml → 類別清單
  - extension/sp_data/senior/{did}/{cid}.xml → 各方言各類別的題目

音檔：https://klokah.tw/extension/sp_senior/sound/{did}/{tid}{typeEn}/{cno}_{odr}[_{ABCDE}].mp3
"""
import requests
import time
...
BASE_DATA  = "https://web.klokah.tw/extension/sp_data/senior"
BASE_SOUND = "https://klokah.tw/extension/sp_senior/sound"
...
TYPE_CONFIG = {
    ...
    # tid=4 senior: single audio (no A/B/C suffix)
    '4': {
    ...
    # tid=7 選擇題三（高中特有）
    '7': {
        'typeEn': 'choice_three', 'odr': 'choiceThreeOrder',
        'fields': [('choiceThreeAb', 'choiceThreeCh', '')],
    },
    # tid=8 唸唸看（高中特有）: A-E 五句
    '8': {
        'typeEn': 'oreal_reading', 'odr': 'oralReadingOrder',
        'fields': [
            ('oralReadingAAb', 'oralReadingACh', 'A'),
            ('oralReadingBAb', 'oralReadingBCh', 'B'),
            ('oralReadingCAb', 'oralReadingCCh', 'C'),
            ('oralReadingDAb', 'oralReadingDCh', 'D'),
            ('oralReadingEAb', 'oralReadingECh', 'E'),
        ],
    },
    '9': {

"""
Wawa 點點樂 — 歌謠：抓取全 42 方言 × 6 課，直接寫入 corpus.db。
Endpoint: https://web.klokah.tw/wawa/php/get_song_data.php?pid={pid}&did={did}
  pid = 1-6，did = 1-43（跳 12）
Response: [{song_num, sentence_num, ab, ch}, ...]（部分方言空陣列）
"""
import requests
import time
from common import get_conn, DIALECTS

BASE_URL = "https://web.klokah.tw/wawa/php/get_song_data.php"
NOTEBOOK = "Wawa歌謠"
SOURCE   = "e樂園"
PIDS     = range(1, 7)
SLEEP    = 0.3
...
def fetch_one(dialect_id: int, pid: int) -> list[dict]:
    ...
    for item in data:
        ...
        song_num = item.get("song_num", "")
        if ab:
            rows.append({
                "unit": f"課{pid}-歌{song_num}",
...
def main():
    ...
    for d_idx, (did, dname) in enumerate(DIALECTS.items(), 1):
        ...
        if dialect_rows:
            ...
            print(f"[{d_idx:02d}/{total}] {dname} ({did})  —")
